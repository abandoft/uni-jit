...
- Added exact IEEE-754 Float64 negation to straight-line and CFG SSA with sign-bit-only interpreter and constant-folding semantics, dead-code cleanup, block-local value numbering, register allocation, and independent AArch64 `FNEG`, x86-64 integer sign-mask XOR, and RISC-V 64 `FSGNJN.D` lowering; replaced QuickJS and PocketPy unary-minus subtraction emulation, qualified signed zero, infinities, and custom NaN payloads across baseline and optimized direct translation, and carried signed-zero checks through counted loops, stock runtimes, deterministic fuzzing, and the installed package.
- Added IEEE-754 Float64 equality and inequality predicates to straight-line and CFG SSA with exact NaN and signed-zero semantics, constant folding, dead-code cleanup, value numbering, register allocation, and independent AArch64, x86-64, and RISC-V 64 lowering; enabled QuickJS numeric `==`, `!=`, `===`, and `!==` plus PocketPy `==` and `!=` in returned Booleans, loop conditions, and guarded break/continue bodies across baseline and optimized live tiers, with deterministic fuzzing and installed-package qualification.
- Added a public compile-time `measure_safepoint_polls` policy that keeps exact native telemetry enabled by default but can remove all counter instructions from latency-critical code; Lua now selects the zero-overhead path because it already derives exact saturating loop hotness from call arguments, while QuickJS and PocketPy retain measured backedges, with straight-line/CFG native opt-out and installed-package qualification.
- Expanded production x86-64 qualification so real Ubuntu GCC and Clang hosts now build and execute the stock QuickJS adapter alongside Lua, PocketPy, the core JIT, performance gates, and installed-package consumer, and Linux ASan/UBSan now covers QuickJS as well; Windows MSVC continues to qualify the core, Lua, and PocketPy because upstream stock QuickJS requires GNU C extensions, closing the previous gap where QuickJS runtime qualification ran only on macOS without misrepresenting MSVC compatibility.
- Added call-scoped safepoint execution telemetry with exact interpreter and native counting on AArch64, x86-64, and RISC-V 64; QuickJS and PocketPy now feed measured counted-loop backedges into saturating tier hotness, expose the metric through their stats APIs, and can asynchronously promote after one 10,000-iteration call while preserving the 64-call policy for straight-line code, with interruption, reset, loop-count, and stock-runtime qualification.
...
- Added single-level structured integer conditions to Lua 5.5 numeric `for` bodies with `==`, `~=`, `<`, `<=`, `>`, and `>=`, exact SSA state merges, guarded arithmetic, `break`, and one-value early `return`, precise induction-guard exit hotness, compact scalar baselines, independently guarded eight-way optimized groups with bounded tails and safepoints, broad stock-Lua differential coverage, explicit rejection of multiple guards and `else` arms, and a balanced three-engine hosted x64 gate retaining the 1.25x stock Lua and 1.10x LuaJIT floors.
- Added guarded runtime-parameter steps to Lua 5.5 numeric `for` compilation, with exact pre-native zero-step errors, direction-independent unsigned remaining-iteration counts, dead-limit pruning that keeps the normal x86-64 loop state register-resident, scalar baselines, eight-way optimized groups and bounded tails across wrapping indices and `math.mininteger`, exhaustive baseline/optimized stock-Lua differential coverage, and a separate balanced three-engine hosted x64 commercial gate retaining the 1.25x stock Lua and 1.10x LuaJIT floors.
...
- Added single-level guarded `break` and `continue` to QuickJS and PocketPy counted loops, carrying exact Float64 local state to loop exit or a shared induction-update block, preserving the no-continue hot backedge shape, rejecting unsupported `else` and multi-statement forms explicitly, and qualifying direct native plus stock-runtime execution.
...
- Added configurable positive per-compilation budgets for parameters, IR nodes and arguments, CFG blocks and edges, exit and stack-map metadata, and native code bytes, enforced before expensive verification and before W^X publication, with 1 MiB untrusted-source ceilings for QuickJS and PocketPy plus public-package rejection coverage.
- Enforced per-version English/Chinese release-note count parity in metadata validation, preventing tag publication when either single-line changelog omits translated updates.
