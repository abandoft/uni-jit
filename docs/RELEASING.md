...
Run `python3 tools/release.py next` to calculate the only valid next version.
...
python3 tools/release.py check --tag v0.1.1
```
...
section with `tools/release.py notes`, and uses that exact `CHANGELOG.md` body
